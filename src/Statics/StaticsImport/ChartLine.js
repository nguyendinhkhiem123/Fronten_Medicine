...
import { getStaticsImportYear } from "api/Statics";

let myChartExport = null;
export default function ChartLine() {
  ...
  useEffect(() => {
    let ctx = document.getElementById("line-chart-export").getContext("2d");
    
    if(myChartExport !== null){
        myChartExport.destroy();
    }
    myChartExport = new Chart(ctx, {
      type: "line",
      data: {
        labels: [
          "January",
    ...
    <Card>
      <CardHeader color="orange" contentPosition="left">
        <h2 className="text-white text-2xl">
          Biểu đô so sánh chi phí nhập hàng hai năm
        </h2>
      </CardHeader>
      <CardBody>
        <div className="mb-10">
        ...
        <div className="relative h-96" id="chart-container">
          <canvas id="line-chart-export"></canvas>
        </div>
      </CardBody>
      {Loading}
    </Card>
